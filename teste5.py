...
MyConfirm = prompts.MyConfirm()
MyPrompt = prompts.MyPrompt()
...
class Bet:
    ...
    def user_generate_bet(self):
        self.bet_numbers, self.bet_stars = user_generate_valid_numbers(bet_numbers,bet_stars)
...
def generate_valid_numbers(bet_numbers,bet_stars):
    numbers = bet_numbers
    stars = bet_stars
    while len(numbers) < 5:
...
def play_game():
    ...
    tickets_menu()
    while True:
        option = int(prompts.MyPrompt.ask("Select an option", choices=[
                     str(key) for key in menu_tickets.keys()]))
        if option == 1:
            ticket = Ticket()
            # if prompt.Confirm.ask("Do you want to auto-generate a random ticket?", default=True):
            if prompts.MyConfirm.ask("Do you want to auto-generate a random ticket?", default=True):
                num_bets = prompts.MyPrompt.ask(f"Enter number of bets")
        ...
        if option == 2:
            ticket = Ticket() 
            # if prompt.Confirm.ask("Do you want to auto-generate a random ticket?", default=True):
            if prompts.MyConfirm.ask("Do you want to to generate a manual ticket?", default=True):
                num_bets = prompts.MyPrompt.ask(f"Enter number of bets")
                user_num_list = []
                while len(user_num_list) < 5:
                    user_numbers = prompts.MyPrompt.ask(f"Enter Numbers {len(user_num_list) + 1}")
                    user_num_list.append(user_numbers)
                    continue
                user_stars_list = []
                while len(user_stars_list) < 2:
                    user_stars = prompts.MyPrompt.ask(f"Enter Stars {len(user_stars_list) + 1}")
                    user_stars_list.append(user_stars)
                    continue
                for _ in range(int(num_bets)):
                    new_bet = Bet()
                    new_bet.user_generate_bet(user_num_list, user_stars_list)
                    ticket.bets.append(new_bet)
